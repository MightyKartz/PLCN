...
- 新增 PLCN v3.0 设备库工作台，可扫描本地、挂载目录和已授权 ADB Android 设备中的 RetroArch 游戏列表。
- 重构 Web UI 信息架构，保留左侧设备/目录、中央游戏库扫描、按需右侧修复预览和底部任务进度。
- 增强行级修复体验：支持主机缩写筛选、封面缩略图、封面状态、人工确认、选中项应用和数据库搜索默认英文名自动检索。
- 优化深色/浅色主题、中英文 UI 切换、按钮含义、表格对齐和窄屏布局。
- 新增设备扫描、UI 信息架构和深色主题回归测试。
