...
当前开发分支增加 Windows/macOS 桌面入口：双击启动、单实例、用户目录存储、原生目录选择、数据包管理、任务取消/重试和本地列表恢复。Windows 安装包/便携 ZIP、macOS Intel/Apple Silicon DMG 由 PR CI 构建，当前为未签名预览。详见 [桌面使用与发布指南](DOC/DESKTOP_GUIDE.md) 和 [本轮实施计划](DOC/DESKTOP_PLAN.md)。
