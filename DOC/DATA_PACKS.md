...
适用于 2026-09-16 的开发分支；尚未发布为新版本。后续桌面改造已提供数据库/任务/恢复 UI，默认配置和缓存已迁至用户目录，详见 [DESKTOP_GUIDE.md](DESKTOP_GUIDE.md)。下文较早的 `.plcn_runtime` 路径是显式构建示例，不再是默认运行位置。
...
- 普通 CSV 目录按内容指纹缓存到用户缓存目录的 `catalogs/`。源数据变化生成新的缓存，正在运行的任务继续使用原快照。
...
恢复列表时先停止相关任务，找到输出中的 `backup_path`，保留当前文件，再用对应备份恢复。ADB 备份在设备上。若 PLCN 异常退出留下 `.plcn-lock`，先确认没有该文件的任务在运行，再移除锁文件。本地列表现可通过“恢复”页面处理，ADB 仍需手动恢复。
