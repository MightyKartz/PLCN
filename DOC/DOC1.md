...
- 如果传统匹配无法覆盖，可以考虑引入 LLM API 作为辅助识别能力。
...
| LLM 能力 | 可提供 API | 当前仅有传统匹配链路，未真实接入 LLM | 若接入，需要先确认成本、隐私和离线降级策略 |
| 匹配置信度 | 设想中未细分 | 前端存在启发式置信度展示，后端缺少统一评分 | 后续由后端产出权威 `match_score` / `match_reason` |
...
4. 最后再评估是否需要 RetroArch 内部插件、外部伴随服务或 LLM 辅助匹配。
